...
def get_coefficients_of_trigonometric_expression(equation : sy.Expr,
                                                 variable: sy.Symbol,
                                                 order_of_trigonometrics: int):
    ...
    equation = equation.replace(sy.sin(variable), (exp_dummy - exp_dummy**-1)/(2*sy.I))

    # collect coefficients
    real_coefficients = []
    complex_coefficients = sy.Poly(equation*exp_dummy**order_of_trigonometrics, exp_dummy).all_coeffs()
...
def get_coefficients_of_polynomial_expression(polynomial: sy.Expr, variable: sy.Expr,
                                              carry: sy.Expr) -> dict[sy.Expr, sy.Expr]:
    coefficients = sy.Poly(polynomial, variable).all_coeffs()
    maximum_power = len(coefficients)
    return {carry*variable**(maximum_power-power-1): coeff for power, coeff in enumerate(coefficients)}
...
class ComponentwiseExpression:
    ...
    def split(self, split_term: sy.Expr) -> None:
        ...
        for component, term in self._expression.items():
             new_components.update({**get_coefficients_of_polynomial_expression(term, split_term, component)})
        self._expression = new_components

    def prune(self) -> None:
        self._expression = {component: term for component, term in self._expression.items()
    ...
    def subs(self, substitutions: dict[sy.Expr, sy.Expr]) -> None:
        self._expression = {component.subs(substitutions): term.subs(substitutions).cancel()
                            for component, term in self._expression.items()}
