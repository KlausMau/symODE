...
def find_solution_of_equation_by_inserting_values(equation : sy.Expr, variable: sy.Symbol,
...
def get_remainder_with_rational_ansatz(system: DynamicalSystem, numerator: sy.Expr, denominator: sy.Expr, ld: sy.Symbol) -> tuple[sy.Expr, ComponentwiseExpression]:
    ...
    equation = dt_numerator*denominator - dt_denominator*numerator - ld*denominator*numerator
    observable = numerator/denominator

    return observable, ComponentwiseExpression(equation)

def get_remainder_with_exponential_ansatz(system: DynamicalSystem, factor: sy.Expr, exponent: sy.Expr, ld: sy.Symbol) -> tuple[sy.Expr, ComponentwiseExpression]:
...
def get_remainder_with_complex_ansatz(system: DynamicalSystem, complex_polynomial: sy.Expr, real_polynomial: sy.Expr, ld: sy.Symbol, beta: sy.Symbol) -> tuple[sy.Expr, ComponentwiseExpression]:
    ...
    equation = dt_complex_polynomial*real_polynomial + beta*dt_real_polynomial*complex_polynomial - ld*complex_polynomial*real_polynomial
